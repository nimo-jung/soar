...
import { DEFAULT_TENANT_VISIBLE_MENU_PATHS, TENANT_MENU_GROUPS, TENANT_MENU_OPTIONS } from '../../constants/tenant-menu';
...
const buildTierDetailsTooltip = (
  tenant: Tenant,
  t: (key: string, options?: Record<string, unknown>) => string,
): string => {
  if (!tenant.tier && !tenant.quota) {
    return '-';
  }
  ...
  if (quota) {
    details.push(`${t('quota.table.epsLimit')}: ${formatTierLimit(quota.epsLimit, '', t('quota.unlimited'))}`);
    details.push(`${t('quota.table.storageQuotaGb')}: ${formatTierLimit(quota.storageQuotaGb, 'GB', t('quota.unlimited'))}`);
    details.push(`${t('quota.table.retentionDays')}: ${formatTierLimit(quota.retentionDays, t('quota.dialog.daysSuffix'), t('quota.unlimited'))}`);
  }

  if (tenant.tier?.description) {
    details.push(`${t('tenants.tiers.description')}: ${tenant.tier.description}`);
  }
...
const TenantsPage: React.FC = () => {
  ...
  const [authSettings, setAuthSettings] = useState<AdminAuthSettings | null>(null);
  const [showMenuManageDialog, setShowMenuManageDialog] = useState(false);
  const [tenantVisibleMenuPaths, setTenantVisibleMenuPaths] = useState<string[]>(DEFAULT_TENANT_VISIBLE_MENU_PATHS);
  const [menuManageTreeExpandedKeys, setMenuManageTreeExpandedKeys] = useState<Record<string, boolean>>({});
  const [savingMenuManage, setSavingMenuManage] = useState(false);
  ...
  const load = async () => {
    ...
    try {
      ...
      setAuthSettings(authSettingsRes.data);
      setTenantVisibleMenuPaths(
        Array.isArray(authSettingsRes.data.tenantVisibleMenuPaths)
          ? authSettingsRes.data.tenantVisibleMenuPaths
  ...
  const menuManageTreeNodes = useMemo<TreeNode[]>(() => TENANT_MENU_GROUPS.map((group) => ({
    key: `group:${group.key}`,
  ...
  const menuGroupByKey = useMemo(
    () => TENANT_MENU_GROUPS.reduce<Record<string, (typeof TENANT_MENU_GROUPS)[number]>>((acc, group) => {
      acc[`group:${group.key}`] = group;
      return acc;
    }, {}),
    [],
  );

  const menuItemByPath = useMemo(
    () => TENANT_MENU_OPTIONS.reduce<Record<string, (typeof TENANT_MENU_OPTIONS)[number]>>((acc, item) => {
      acc[item.path] = item;
      return acc;
    }, {}),
    [],
  ...
  const menuManageTreeSelectionKeys = useMemo<TreeSelectionState>(
    () => {
      const selectedPathSet = new Set(tenantVisibleMenuPaths);

      const keys = tenantVisibleMenuPaths.reduce<TreeSelectionState>((acc, path) => {
        acc[path] = { checked: true };
        return acc;
      }, {});

      TENANT_MENU_GROUPS.forEach((group) => {
        const groupKey = `group:${group.key}`;
        const total = group.items.length;
        const checkedCount = group.items.filter((item) => selectedPathSet.has(item.path)).length;
  ...
  const applyMenuTreeSelection = (selection: TreeSelectionState) => {
    ...
    const ordered = TENANT_MENU_OPTIONS
      .map((item) => item.path)
      .filter((path) => selectedPathSet.has(path));

    setTenantVisibleMenuPaths(ordered);
  };

  const renderMenuTreeNode = (node: TreeNode) => {
    const nodeKey = String(node.key ?? '');
    const isGroupNode = nodeKey.startsWith('group:');

    if (isGroupNode) {
      const group = menuGroupByKey[nodeKey];
      const selectedCount = group
        ? group.items.filter((item) => tenantVisibleMenuPaths.includes(item.path)).length
        : 0;
      const totalCount = group?.items.length ?? 0;
      const isGroupSelected = totalCount > 0 && selectedCount === totalCount;
      const iconClass = group?.icon;

      return (
        <span className={`tenant-menu-node tenant-menu-node-group ${isGroupSelected ? 'is-selected' : ''}`}>
          {iconClass ? <i className={`tenant-menu-node-icon ${iconClass}`} /> : null}
          <span className="tenant-menu-node-label">{node.label}</span>
          <span className="tenant-menu-node-count">{selectedCount}/{totalCount}</span>
        </span>
      );
    }

    const isSelected = tenantVisibleMenuPaths.includes(nodeKey);
    const iconClass = menuItemByPath[nodeKey]?.icon;

    return (
      <span className={`tenant-menu-node ${isSelected ? 'is-selected' : ''}`}>
        {iconClass ? <i className={`tenant-menu-node-icon ${iconClass}`} /> : null}
        <span className="tenant-menu-node-label">{node.label}</span>
      </span>
    );
  ...
  const openMenuManageDialog = () => {
    setTenantVisibleMenuPaths(
      authSettings?.tenantVisibleMenuPaths && authSettings.tenantVisibleMenuPaths.length > 0
        ? authSettings.tenantVisibleMenuPaths
        : DEFAULT_TENANT_VISIBLE_MENU_PATHS,
    );
    setMenuManageTreeExpandedKeys(
      TENANT_MENU_GROUPS.reduce<Record<string, boolean>>((acc, group) => {
        acc[`group:${group.key}`] = true;
        return acc;
      }, {}),
    );
    setShowMenuManageDialog(true);
  };

  const handleSaveMenuManage = async () => {
    if (tenantVisibleMenuPaths.length === 0) {
      openResultDialog(
        t('tenants.menuManagement.validationTitle'),
        t('tenants.menuManagement.validationAtLeastOne'),
      );
      return;
    }

    const currentAuthSettings = authSettings ?? (await api.get<AdminAuthSettings>('/admin/auth-settings')).data;

    setSavingMenuManage(true);
    try {
      const response = await api.patch<AdminAuthSettings>('/admin/auth-settings', {
        ...currentAuthSettings,
        tenantVisibleMenuPaths,
      });

      setAuthSettings(response.data);
      setTenantVisibleMenuPaths(response.data.tenantVisibleMenuPaths ?? tenantVisibleMenuPaths);
      setShowMenuManageDialog(false);
      openResultDialog(t('tenants.menuManagement.savedTitle'), t('tenants.menuManagement.savedDetail'));
    } catch (error: unknown) {
      const { message } = getApiErrorStatusAndMessage(error);
      openResultDialog(
        t('tenants.menuManagement.saveFailedTitle'),
        message ?? t('tenants.menuManagement.saveFailedDetail'),
      );
    } finally {
      setSavingMenuManage(false);
    }
  };
  ...
  return (
    <div className="admin-page tenants-page">
      ...
      />
      <Tooltip
        target=".tenant-tier-name-cell[data-pr-tooltip]"
        className="tenant-tier-tooltip"
        showDelay={120}
        hideDelay={80}
      />
      <ConfirmDialog />
      <div className="admin-page-header page-header">
        ...
        <div className="flex gap-2">
          <Button
            label={t('tenants.menuManagement.button')}
            icon="pi pi-list-check"
            outlined
            severity="secondary"
            onClick={openMenuManageDialog}
          />
          <Button
      ...
      <div className="admin-table-shell">
        ...
        >
        {isFieldVisible('id') && (
          <Column field="id" header={t('tenants.table.id')} style={{ width: '5rem' }} bodyClassName="text-right" headerClassName="text-right" />
        )}
        {isFieldVisible('slug') && (
          <Column
            field="slug"
        ...
        {isFieldVisible('tier') && (
          <Column
            ...
            style={{ width: '14rem' }}
            bodyClassName="tenant-col-tier"
            body={(row: Tenant) => (
              <span
                className="tenant-tier-name-cell"
                data-pr-tooltip={buildTierDetailsTooltip(row, t)}
        ...
        {isFieldVisible('ipCidr') && (
          <Column
            ...
            style={{ width: '10rem' }}
            bodyClassName="tenant-col-ip"
            body={(row: Tenant) => row.ipCidr || '-'}
          />
        )}
        {isFieldVisible('contactEmail') && (
          <Column field="contactEmail" header={t('tenants.table.contactEmail')} style={{ width: '13rem' }} bodyClassName="tenant-col-email" />
        )}
      ...
      <Dialog
        header={t('tenants.menuManagement.title')}
        visible={showMenuManageDialog}
      ...
      >
        <div className="flex flex-column gap-3 pt-2">
          <p className="m-0 text-color-secondary text-sm">
            {t('tenants.menuManagement.description')}
          </p>
          <div className="flex justify-content-end gap-2">
            <Button
              ...
              text
              severity="secondary"
              label={t('tenants.menuManagement.clearAll')}
              onClick={() => {
                setTenantVisibleMenuPaths([]);
              }}
            />
            <Button
              ...
              text
              severity="secondary"
              label={t('tenants.menuManagement.selectAll')}
              onClick={() => {
                setTenantVisibleMenuPaths(DEFAULT_TENANT_VISIBLE_MENU_PATHS);
              }}
            />
          </div>
          <div className="tenant-menu-tree-wrap">
            <Tree
              value={menuManageTreeNodes}
              selectionMode="checkbox"
              selectionKeys={menuManageTreeSelectionKeys}
              nodeTemplate={renderMenuTreeNode}
              expandedKeys={menuManageTreeExpandedKeys}
              onToggle={(event) => {
          ...
          <div className="flex justify-content-end gap-2">
            ...
            <Button
              label={t('common.save')}
              icon="pi pi-save"
              loading={savingMenuManage}
              onClick={() => {
                void handleSaveMenuManage();
              }}
            />
          </div>
        </div>
      ...
      <Dialog
        ...
        className="tenant-dialog tenant-create-dialog"
