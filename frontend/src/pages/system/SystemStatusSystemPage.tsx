...
const SystemStatusPage: React.FC = () => {
  ...
  const [resultDialog, setResultDialog] = useState({
    ...
    title: '',
    message: '',
    tone: 'info' as ResultDialogTone,
  });

  const openResultDialog = (title: string, message: string, tone: ResultDialogTone = 'info') => {
    setResultDialog({ visible: true, title, message, tone });
  };
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      openResultDialog(
        t('systemStatus.resultDialog.failedTitle'),
        extractApiMessage(error) || t('systemStatus.resultDialog.loadFailed'),
        'error',
      );
    } finally {
      setLoading(false);
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      openResultDialog(t('systemStatus.resultDialog.successTitle'), t('systemStatus.checkDone'), 'success');
      await loadAll();
    } catch (error: unknown) {
      openResultDialog(
        t('systemStatus.resultDialog.failedTitle'),
        extractApiMessage(error) || t('systemStatus.resultDialog.checkFailed'),
        'error',
      );
    } finally {
      setChecking(false);
  ...
  const handleResolve = async (id: number) => {
    try {
      ...
      openResultDialog(t('systemStatus.resultDialog.successTitle'), t('systemStatus.resultDialog.resolveSuccess'), 'success');
      void loadAlerts(alertsPage);
    } catch (error: unknown) {
      openResultDialog(
        t('systemStatus.resultDialog.failedTitle'),
        extractApiMessage(error) || t('systemStatus.resultDialog.resolveFailed'),
        'error',
      );
    }
  };
