...
const BillingSystemPage: React.FC = () => {
  ...
  const [draftTierCode, setDraftTierCode] = useState('');
  ...
  const loadTenants = async () => {
    ...
    setTenantOptions(response.data.map((tenant) => ({ label: tenant.name, value: tenant.id })));
  };
  ...
  const loadUsage = async (
    ...
    setLoading(true);
    try {
      const response = await api.get<UsageResponse>('/admin/billing/usage', {
        params: {
  ...
  const loadPricingPolicies = async () => {
    ...
    try {
      const response = await api.get<PricingPolicyResponse>('/admin/billing/pricing-policies');
      setPricingPolicies(response.data.items);
    } finally {
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const response = await api.get('/admin/billing/usage/export', {
        params: {
  ...
  const setPolicyNumber = (targetTier: string, key: keyof Omit<PricingPolicyRow, 'tierCode' | 'currency'>, value: number) => {
    setPricingPolicies((prev) => prev.map((policy) => (
      policy.tierCode === targetTier ? { ...policy, [key]: Number.isFinite(value) ? value : 0 } : policy
    )));
  };

  const setPolicyCurrency = (targetTier: string, value: string) => {
    setPricingPolicies((prev) => prev.map((policy) => (
      policy.tierCode === targetTier ? { ...policy, currency: value.toUpperCase() } : policy
    )));
  };
  ...
  const handleSavePricingPolicies = async () => {
    ...
    try {
      ...
      const response = await api.patch<PricingPolicyResponse>('/admin/billing/pricing-policies', payload);
      setPricingPolicies(response.data.items);
    } finally {
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        <h1 className="admin-page-title">{t('billing.title')}</h1>
        <div className="admin-actions-row">
          ...
          />
          <Button type="button" icon="pi pi-refresh" label={t('billing.actions.refresh')} outlined loading={loading} onClick={() => void loadUsage(1, limit)} />
          <Button type="button" icon="pi pi-database" label={t('billing.actions.collectUsage')} outlined loading={collecting} onClick={() => void handleCollectUsage()} />
          <Button type="button" icon="pi pi-download" label={t('billing.actions.exportCsv')} outlined loading={exporting} onClick={() => void handleExport()} />
        </div>
      </div>
      ...
      <Dialog
        ...
        footer={(
          <div className="flex justify-content-end gap-2">
            <Button type="button" icon="pi pi-check" label={t('billing.actions.applyFilters')} onClick={applyFilters} />
          </div>
        )}
      >
        <div className="grid pt-2">
          <div className="col-12">
            ...
            <Dropdown
              ...
              onChange={(event) => setDraftTenantId(event.value as number | null)}
              className="w-full"
            />
          </div>
          <div className="col-12">
            <label className="admin-form-label">{t('billing.filters.tier')}</label>
            <Dropdown value={draftTierCode} options={tierOptions} onChange={(event) => setDraftTierCode(String(event.value ?? ''))} className="w-full" />
          </div>
          <div className="col-12 md:col-6">
            <label className="admin-form-label">{t('billing.filters.from')}</label>
            <InputText type="date" value={draftFrom} onChange={(event) => setDraftFrom(event.target.value)} className="w-full" />
          </div>
          <div className="col-12 md:col-6">
            <label className="admin-form-label">{t('billing.filters.to')}</label>
            <InputText type="date" value={draftTo} onChange={(event) => setDraftTo(event.target.value)} className="w-full" />
          </div>
        </div>
      </Dialog>
      ...
      <div className="mt-3">
        <div className="flex justify-content-between align-items-center mb-2">
          ...
          <Button
            ...
            onClick={() => void handleSavePricingPolicies()}
          />
        </div>
          ...
          <CommonDataTable value={pricingPolicies} loading={loadingPricing} className="admin-table">
            ...
            <Column
              ...
              body={(row: PricingPolicyRow) => (
                <InputNumber value={row.baseFee} min={0} minFractionDigits={2} maxFractionDigits={2} mode="decimal" useGrouping={false} className="w-full" onValueChange={(event) => setPolicyNumber(row.tierCode, 'baseFee', event.value ?? 0)} />
              )}
            />
            <Column
              header={t('billing.pricing.columns.includedEps')}
              body={(row: PricingPolicyRow) => (
                <InputNumber value={row.includedEps} min={0} minFractionDigits={0} maxFractionDigits={2} mode="decimal" useGrouping={false} className="w-full" onValueChange={(event) => setPolicyNumber(row.tierCode, 'includedEps', event.value ?? 0)} />
              )}
            />
            <Column
              header={t('billing.pricing.columns.epsOveragePer100')}
              body={(row: PricingPolicyRow) => (
                <InputNumber value={row.epsOveragePer100} min={0} minFractionDigits={0} maxFractionDigits={4} mode="decimal" useGrouping={false} className="w-full" onValueChange={(event) => setPolicyNumber(row.tierCode, 'epsOveragePer100', event.value ?? 0)} />
              )}
            />
            <Column
              header={t('billing.pricing.columns.storageOveragePerGb')}
              body={(row: PricingPolicyRow) => (
                <InputNumber value={row.storageOveragePerGb} min={0} minFractionDigits={0} maxFractionDigits={4} mode="decimal" useGrouping={false} className="w-full" onValueChange={(event) => setPolicyNumber(row.tierCode, 'storageOveragePerGb', event.value ?? 0)} />
              )}
            />
            <Column
              header={t('billing.pricing.columns.logPerMillion')}
              body={(row: PricingPolicyRow) => (
                <InputNumber value={row.logPerMillion} min={0} minFractionDigits={0} maxFractionDigits={4} mode="decimal" useGrouping={false} className="w-full" onValueChange={(event) => setPolicyNumber(row.tierCode, 'logPerMillion', event.value ?? 0)} />
              )}
            />
            <Column
              header={t('billing.pricing.columns.currency')}
              body={(row: PricingPolicyRow) => (
                <InputText value={row.currency} maxLength={10} className="w-full" onChange={(event) => setPolicyCurrency(row.tierCode, event.target.value)} />
              )}
            />
          </CommonDataTable>
        </div>
