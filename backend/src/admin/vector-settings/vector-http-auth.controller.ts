...
@ApiTags('Internal - Vector HTTP Auth')
@Controller('internal/vector-http-auth')
export class VectorHttpAuthController {
  constructor(private readonly vectorSettingsService: VectorSettingsService) {}

  @Get('validate')
  @ApiOperation({ summary: 'Vector HTTP 인증 검증' })
  async validate(
    ...
    const allowed = await this.vectorSettingsService.validateVectorHttpAuth(
      tenantSlug ?? '',
      sourceId ?? '',
      authorization,
      authToken,
    ...
    if (!allowed) {
      throw new UnauthorizedException('vector http auth failed');
    }
